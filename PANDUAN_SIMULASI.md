...
- **Tambah Kategori**: `POST /api/categories`
  ```json
  { "name": "Electronics", "description": "Barang elektronik" }
  ```
- **Tambah Customer**: `POST /api/customers`
  ```json
  { "name": "Budi", "email": "budi@mail.com", "address": "Jakarta" }
  ```
- **Lihat Data**: 
...
- **Tambah Produk**: `POST /api/products`
  ```json
  { "name": "Laptop", "price": 10000000, "stock": 50, "categoryId": 1 }
  ```
...
```
*(Catatan: Pastikan urutan TRUNCATE pada `order_management_db` sesuai agar tidak terjadi error relasi).*
